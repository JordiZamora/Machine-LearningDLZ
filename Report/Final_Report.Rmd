...
We have focused our classification efforts on three classification methods: Random Forest, Nearest Neighbours, and Support vector machines.
...
**Nearest Neighbours with weighted binary data**
...
We conclude that the best method we could find to classify the forest covertypes was the kNN with k=1 by defining the binary variables $\in \lbrace 0, 91 \rbrace$. That method leads to our best Kaggle submission with a $92.52$ of accuracy in the $10\%$ of the test set. Our approach relies on the fact that the binary variables are underepresented in the calculation of the euclidean distances used in kNN. Since the scaling of the continuous variables didn't give us good results we decided to scale all the binary variables with a fixed weight. This method gives the best compromise between the relevance of the continuous features and the binary features.

Other methods were discarded in the process that performed slightly worst than that. Specially disapointing was the inclussion of the SVD and PCA as preprocessing. The bad results got whit this methods was probably due to the fact that they are suposed to minimize the dispersion of the data. That is quite different from the optimization of the misclassification error, so from this point of view it makes not much sense to try to apply this techniques in the way we did.